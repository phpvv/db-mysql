...
class Driver implements \VV\Db\Driver\Driver
{
    ...
    public function connect(string $host, string $user, string $passwd, ?string $scheme, ?string $charset): Connection
    {
        ...
        if (!$res) {
            throw new \VV\Db\Exceptions\SqlExecutionError('Can\'t set SQL_MODE', 0, self::createMysqliError($mysqli));
        }
     ...
     */
    static function createMysqliError(\mysqli $mysqli, string $queryString = null): ?MysqliError
    {
